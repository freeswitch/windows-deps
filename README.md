...
- **libjpeg is IJG's own Windows build.** `jconfig.h` is `jconfig.vc`, as
  `install.txt` says for Visual C++, and the file list is the one in
  `makejvcx.v16`, the Visual Studio project the release carries for the
  library -- the same 46 files, `jmemnobs.c` among them, that
  `libs\win32\libjpeg\libjpeg.2017.vcxproj` compiled out of `jpegsrc.v8d.tar.gz`
